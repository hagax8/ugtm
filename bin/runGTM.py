...
parser.add_argument('--data', \
                    ...
                    (must be a similarity matrix if --model kGTM is set, otherwise not)', \
                    dest='filenamedat')
...
parser.add_argument('--model', \
                    help='GTM model, kernel GTM model, SVM, PCA or comparison between: \
                          GTM, kGTM, LLE and tSNE for simple visualization, \
                          GTM and SVM for regression or classification (--crossvalidate); \
...
parser.add_argument('--crossvalidate',
                    help='show best l (regularization coefficient) and s (RBF width factor) \
                          for classification or regression, \
                          with default grid size parameter k = sqrt(5*sqrt(Nfeatures))+2) \
                          and RBF grid size parameter m = sqrt(k); \
                          you can also set the 4 parameters and run only one model with \
...
parser.add_argument('--test', \
                    help='test data; only available for GTM classification in this script \
                          (define training data with --data and \
                          training labels with --labels with --labeltype discrete)', \
                    dest='test')
parser.add_argument('--missing_strategy', \
                    help='missing data strategy, missing values encoded by NA; default is median', \
                    const='median', \
                    type=str, \
                    default='median', \
                    nargs='?', \
...
parser.add_argument('--predict_mode', \
                    help='predict mode for GTM classification: \
                          default is bayes for an equiprobable class prediction, \
                          you can change this to knn; knn is the only one available \
                          for PCA and t-SNE, this option is only useful for GTM', \
                    const='bayes', \
                    type=str, \
                    default='bayes', \
                    nargs='?', \
...
parser.add_argument('--prior', \
                    help='type of prior for GTM classification map and prediction model: \
                          you can choose equiprobable classes \
                          (prior any class=1/nClasses) \
                          or to estimate classes from the training set \
                          (prior class 1 = \
                          sum(class 1 instances in train)/sum(instances in train))', \
                    const='equiprobable', \
                    type=str, \
...
parser.add_argument('--regularization', \
                    help='set regularization factor, default: 0.1; \
                          set this to -1 to crossvalidate when using --crossvalidate', \
                    type=float, \
                    dest='regularization', \
                    default=0.1, \
                    nargs='?', \
...
parser.add_argument('--rbf_width_factor', \
                    help='set RBF (radial basis function) width factor, default: 0.3; \
                          set this to -1 to crossvalidate when using --crossvalidate', \
                    type=float, \
...
parser.add_argument('--random_state', \
                    help='change random state for map initialization (default is 5)', \
                    const=1234, \
                    type=int, \
                    default=1234, \
                    nargs='?', \
...
parser.add_argument('--representation', \
                    help='type of representation used for GTM: modes or means', \
                    dest='representation', \
                    const='modes', \
                    type=str, \
                    default='modes', \
...
parser.add_argument('--kernel', \
                    help='type of kernel for Kernel GTM - default is euclidean', \
                    dest='kernel', \
                    const='euclidean', \
                    type=str, \
                    default='euclidean', \
                    nargs='?', \
                    choices=['euclidean','laplacian','jaccard','cosine','linear'])
...
if args.usetest == 's':
    data,labels = sklearn.datasets.samples_generator.make_s_curve(500, random_state=args.random_state)
elif args.usetest == 'swiss':
    data,labels = sklearn.datasets.make_swiss_roll(n_samples=2000,random_state=args.random_state)
...
#########in case it's a train/test experiment for GTM classification
    ...
    if args.filenametestids is not None:
        testids = np.genfromtxt(args.filenametestids, delimiter="\t", dtype=str)
    else:
        testids = ""
    prediction=ugtm.advancedGTC(train=data,labels=labels,test=test,doPCA=args.pca, \
    ...
    prediction['optimizedModel'].plot_html(ids=ids,plot_arrows=True, \
                                           title="GTM",discrete=discrete,output=args.output)
...
###################################################
    ...
    if args.model != 'GTM' and args.model != 'kGTM':
        data=ugtm.pcaPreprocess(data=data,doPCA=args.pca,n_components=args.n_components, \
                                missing=args.missing, \
                                missing_strategy=args.missing_strategy, \
                                random_state=args.random_state)
    ...
    #########if it's for GTM visualization
        ...
        gtm.plot_multipanel(labels=labels,output=args.output,discrete=discrete)    
        gtm.plot_html(labels=labels,ids=ids,discrete=discrete,output=args.output)
        exit
    ...
    ###########if it's for kGTM visualization
        ...
        #make pdf
        kgtm.plot_multipanel(labels=labels,output=args.output,discrete=discrete)
        #interactive plot
    ...
    ##########if it's to compare GTM, PCA, LLE and t_SNE visualizations
        ...
        ax.scatter(gtm.matMeans[:, 0], gtm.matMeans[:, 1], c=labels, cmap=plt.cm.Spectral)
        plt.axis('tight')
        plt.xticks([]), plt.yticks([])
        plt.title('GTM')
        ax = fig.add_subplot(337)
        if discrete:
            ugtm.plotClassMap(optimizedModel,labels)
        else:
            ugtm.plotLandscape(optimizedModel,labels)
        ...
        kgtm = ugtm.runkGTM(data = matK, k = k, m = m, s = s, l = l, niter = niter, \
                                    ...
                                    n_components = args.n_components, missing = args.missing, \
                                    missing_strategy = args.missing_strategy, \
                                    random_state = args.random_state)
        print("The estimated feature space dimension is: ", initialModel.n_dimensions)
        end = time.time()
        elapsed = end - start
        print("time taken for kGTM: ",elapsed)
        ax = fig.add_subplot(335)
        ax.scatter(kgtm.matMeans[:, 0], kgtm.matMeans[:, 1], c=labels, cmap=plt.cm.Spectral)
        plt.axis('tight')
        plt.xticks([]), plt.yticks([])
        ...
        start = time.time() 
        data_r, err = manifold.locally_linear_embedding(data, n_neighbors=12,n_components=2)
        end = time.time()
        elapsed = end - start
    ...
    print("Sorry. Could not guess what you wanted. \
           Remember to define --model and (--data and --labels) or --model and --usetest.")
    exit
