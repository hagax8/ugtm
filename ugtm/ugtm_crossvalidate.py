...
def crossvalidateGTC(data,labels,n_neighbors=1,representation="modes", \
                     ...
                     random_state=1234,predict_mode="bayes",prior="equiprobable", \
		...
		for l in lvec:
			...
			for j in range(n_repetitions):
				...
				for train_index, test_index in ss.split(data):
					...
					prediction=GTC(train=train,labels=labels[train_index], \
			...
			for i in range(0,nClasses):
				...
				hclass[i] =  seclass[i] * t._ppf((1+0.95)/2., len(recallclassvec[:,i])-1)
				hprecisionclass[i] =  seprecisionclass[i] \
                                                      * t._ppf((1+0.95)/2., len(precisionclassvec[:,i])-1)
				hf1class[i] =  sef1class[i] * t._ppf((1+0.95)/2., len(f1classvec[:,i])-1)
		...
		for l in lvec:
			...
			for j in range(n_repetitions):
				...
				for train_index, test_index in ss.split(data):
					train = np.copy(data[train_index])
					test = np.copy(data[test_index])
					prediction = GTR(train=train,labels=labels[train_index], \
                                                         test=test,k=k,m=m,s=s,l=l, \
                                                         n_neighbors=n_neighbors,niter=niter, \
                                                         representation=representation, \
                                                         doPCA=doPCA,n_components=n_components, \
	...
	for c in Kvec:
		...
		for j in range(n_repetitions):
			...
			for train_index, test_index in ss.split(data):
				...
				y_pred = np.append(y_pred,predictNNSimple(processed.train,processed.test, \
                                                   labels[train_index],c,"classification"))
				y_true = np.append(y_true,labels[test_index])
			recall = recall_score(y_true, y_pred, average='weighted')
			precision = precision_score(y_true, y_pred, average='weighted')
			f1 = f1_score(y_true, y_pred, average='weighted')
	...
	for c in Kvec:
		...
		for j in range(n_repetitions):
			...
			for train_index, test_index in ss.split(data):
				train=np.copy(data[train_index])
				test=np.copy(data[test_index])
				processed=processTrainTest(train,test, \
                                                           doPCA,n_components, \
                                                           missing,missing_strategy)
				y_pred = np.append(y_pred,predictNNSimple(processed.train,processed.test, \
                                                                          labels[train_index], \
		...
		for eps in EpsVec:
			...
			for j in range(n_repetitions):
				...
				for train_index, test_index in ss.split(data):
					train = np.copy(data[train_index])
					test = np.copy(data[test_index])
					processed=processTrainTest(train,test,doPCA, \
...
def whichExperiment(data,labels,args,discrete=False):
	if discrete==True and args.model=='GTM':
		decide = 'crossvalidateGTC'
	elif discrete==False and args.model=='GTM':
		decide = 'crossvalidateGTR'
	elif discrete==True and args.model=='SVM':
		decide = 'crossvalidateSVC'
	elif discrete==False and args.model=='SVM':
		decide = 'crossvalidateSVR'
	elif discrete==True and args.model=='SVMrbf':
		decide = 'crossvalidateSVCrbf'
	elif discrete==True and args.model=='PCA':
		decide = 'crossvalidatePCAC'
	elif discrete==False and args.model=='PCA':
		decide = 'crossvalidatePCAR'
	elif discrete==True and args.model=='compare':
		decide = 'comparecrossvalidateC'
	elif discrete==False and args.model=='compare':
