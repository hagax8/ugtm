...
def advancedGTC(train,labels,test,n_neighbors=1,representation="modes", \
                niter=200,k=0,m=0,doPCA=False,n_components=-1,missing=False, \
	...
	processed = processTrainTest(train,test,doPCA, \
                                     n_components,missing,missing_strategy)
	...
	prediction = advancedPredictBayes(optimizedModel,labels,processed.test,prior)
	return prediction

def advancedPredictBayes(optimizedModel,labels,new_data,prior="equiprobable"):
	predicted = {}
	...
	predicted["group_probabilities"] = np.matmul(predicted["group_projections"], \
                                                     activityModel)
...
def printClassPredictions(prediction,output):
	...
	for i in range(len(prediction["uniqClasses"])):
		...
		count = count + 1
	predvec = [prediction["uniqClasses"][j] for j in prediction["indiv_predictions"]]
	...
	np.savetxt(fname=output+"_indiv_predictions.csv", \
                   X=prediction["indiv_predictions"],delimiter=",",header=string,fmt='%s')
...
def GTC(train,labels,test,k=0,m=0,s=0.3,l=0.1,n_neighbors=1,niter=200, \
	...
	if predict_mode=="knn":
		prediction = predictNN(optimizedModel,labels,processed.test, \
                                       "classification",n_neighbors,representation,prior)
	elif predict_mode=="bayes":
		prediction = predictBayes(optimizedModel,labels,processed.test,prior)
	return prediction
...
def GTR(train,labels,test,k=0,m=0,s=0.3,l=0.1,n_neighbors=1,niter=200, \
	...
	prediction = predictNN(optimizedModel=optimizedModel,labels=labels, \
                               ...
                               n_neighbors=n_neighbors,representation=representation)
	return prediction
