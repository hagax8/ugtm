...
from .ugtm_plot import plot_html
...
class OptimizedGTM(object):
	...
	def write(self,output="output"):
		np.savetxt(fname=output+"_responsibilities.csv",X=self.matR,delimiter=",")
		np.savetxt(fname=output+"_coordinates.csv",X=self.matMeans,delimiter=",")
		np.savetxt(fname=output+"_modes.csv",X=self.matModes,delimiter=",")
		print("")
		print("Wrote to disk:")
		print("")
		print("%s: responsibilities, which represent each individual's encoding \
	...
	def write_all(self,output="output"):
		...
		np.savetxt(fname=output+"_parametersMatrix.csv",X=self.matW,delimiter=",")
	...
	def plot(self,labels=None,title="",output="output", \
                 discrete=False,pointsize=1,alpha=0.3):
	...
	def plot_html_projection(self,projections,labels=None, \
		...
		plot_html_GTM_projection(optimizedModel=self,projections=projections, \
                                         labels=labels,ids=ids,plot_arrows=plot_arrows, \
                                         title=title,discrete=discrete,output=output, \
                                         pointsize=pointsize,alpha=alpha, \
                                         do_interpolate=do_interpolate)
