...
def plot_pdf(coordinates,labels=None,title="",output="output", \
	...
	scatter = ax.scatter(coordinates[:, 0].tolist(),coordinates[:, 1].tolist(), \
                             c=colvec, s=20*pointsize,alpha=alpha,cmap=plt.cm.Spectral, \
                             edgecolor='black')
	plt.xlim(-1.1, 1.1)
	plt.ylim(-1.1, 1.1)
...
def plot_html(coordinates,labels=None,ids=None,title="plot", \
	...
	ax.grid(color='white', linestyle='solid')
	ax.set_title(title,size=30)
	pointsidx = ['point {0}'.format(i + 1) for i in range(coordinates.shape[0])]
	scatter = ax.scatter(coordinates[:, 0].tolist(),coordinates[:, 1].tolist(), \
                             c=colvec, s=20*pointsize,alpha=alpha,cmap=plt.cm.Spectral, \
                             edgecolor='black')
...
def plot_html_GTM_projection(optimizedModel,projections,labels=None, \
                             ids=None,plot_arrows=True,title="GTM_projection", \
                             discrete=False,output="output",pointsize=1,alpha=0.3, \
                             do_interpolate=True):
	if labels is None:
		colvec = "black"
	elif discrete:
	...
	fig, ax = plt.subplots(subplot_kw=dict(facecolor='#EEEEEE'))
	if discrete and labels is not None:
		plotClassMap(optimizedModel,label_numeric,do_interpolate=do_interpolate)
	elif labels is not None:
		plotLandscape(optimizedModel,label_numeric,do_interpolate=do_interpolate)
	...
	if ids is not None:
		...
		tooltip = mpld3.plugins.PointLabelTooltip(scatter,labels=list(pointsidx))
	mpld3.plugins.connect(fig, tooltip)
	mpld3.save_html(fig,output+".html")
	plt.close(fig)
	print("\nWrote html plot to disk: %s\n" % (output+".html"))
...
def plotLandscape(optimizedModel,labels,do_interpolate=True):
	...
	z = landscape(optimizedModel,labels)
	if do_interpolate:
	...
	plt.scatter(optimizedModel.matMeans[:,0], optimizedModel.matMeans[:,1], \
                    20, labels, cmap=plt.cm.Spectral,edgecolor='black',marker="o")
	plt.title('Landscape')
	plt.xlim(-1.1, 1.1)
	plt.ylim(-1.1, 1.1)
	plt.colorbar()
...
def plotClassMap(optimizedModel,labels,prior="equiprobable",do_interpolate=True):
	k = math.sqrt(optimizedModel.matX.shape[0])
	n = 100
	x = optimizedModel.matX[:,0]
	y = optimizedModel.matX[:,1]
	uniqClasses, label_numeric = np.unique(labels, return_inverse=True)
	z = classMap(optimizedModel,label_numeric,prior).activityModel
	if do_interpolate:
		ti = np.linspace(-1.1, 1.1, n)
		XI, YI = np.meshgrid(ti, ti)
		f = interpolate.NearestNDInterpolator(optimizedModel.matX,z)
	...
	plt.scatter(optimizedModel.matMeans[:,0], optimizedModel.matMeans[:,1], \
                    20, label_numeric, cmap=plt.cm.Spectral,edgecolor='black',marker="o")
	plt.title('Class Map')
	plt.xlim(-1.1, 1.1)
	plt.ylim(-1.1, 1.1)
	plt.axis('tight')
...
def plotClassMapNoPoints(optimizedModel,labels,prior="equiprobable",do_interpolate=True):
	k = math.sqrt(optimizedModel.matX.shape[0])
	n=100
	x = optimizedModel.matX[:,0]
	y = optimizedModel.matX[:,1]
