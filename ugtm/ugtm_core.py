...
def optimWMatrix(matR, matPhiMPlusOne, matG, data, betaInv, l):
	...
	matW = np.transpose(np.dot(np.dot(np.dot(Ginv, np.transpose(matPhiMPlusOne)),matR),data))
	return(matW)
...
def computelogLikelihood(matP,betaInv,n_dimensions):
	...
	constante=np.longdouble(np.power(((1/betaInv)/(2*np.pi)), \
                                         np.minimum(n_dimensions/2,placeholder)))
	LogLikelihood = np.sum(np.log(np.maximum(np.sum(constante*matP,axis=0)*prior, \
